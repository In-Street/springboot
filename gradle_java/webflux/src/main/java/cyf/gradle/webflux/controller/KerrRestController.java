...
import cyf.gradle.webflux.service.KerrRestService;
 ...
 *
 * @author Cheng Yufei
 * @create 2018-04-08 16:00
 **/
@RestController
@RequestMapping("/flux")
public class KerrRestController {

    @Autowired
    private KerrRestService kerrRestService;
...
//        return Flux.just("Hello World");
    ...
    @GetMapping("/kerr")
    public Flux<Kerr> getKerrs(Integer[] ids) {
        Flux<Integer> idsFlux = Flux.just(ids);
        return kerrRestService.getKerrsByIds(idsFlux);
    }

    @GetMapping("/kerrs")
    public Flux<Kerr> getKerrs() {
        return kerrRestService.getKerrs();
    }

    @GetMapping("/kerr/{id}")
    public Mono<Response> getKerrById(@PathVariable Integer id) {
        return kerrRestService.getKerrById(id);
    }
}
